...
/// 可复用的遥控控件面板：雷达安全状态条 + 虚拟摇杆 + 横移开关 + STOP 急停。
/// 遥控页与建图模式页共用（建图中开车绕图）。返回 Column（不含 Scaffold）。
///
/// **安全底线 deadman**：松手 / 移除本控件(dispose) / 切后台 / 触摸取消 → 立即发零并停发送循环，
/// 绝不让车卡着上一个速度继续跑。避障在机器人本地实时跑，这端只负责不“甩手不管”。
class TeleopControlPanel extends StatefulWidget {
  const TeleopControlPanel({super.key});

  @override
  State<TeleopControlPanel> createState() => _TeleopControlPanelState();
}

class _TeleopControlPanelState extends State<TeleopControlPanel>
  ...
  @override
  void dispose() {
    // deadman：移除本控件务必发零（fire-and-forget，teleopClient 是单例，dispose 后仍能完成）。
    _sendTimer?.cancel();
    _statusTimer?.cancel();
    _sendZeroBurst();
  ...
  // ---- 显示辅助 ----
  bool get _stale => _status.state == 'unknown' || _status.ageMs > _staleMs;
        ...
        return Column(
          children: [
            ...
            SwitchListTile(
              ...
              value: _strafe,
              onChanged: (v) {
                _stopDriving(sendZero: true); // 切模式即归零，避免残留上一模式的 vy/wz
                setState(() {
                  _strafe = v;
                  _resetToken++;
                });
