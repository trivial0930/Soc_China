...
def build_prompt(request: CognitionRequest, with_image: bool = True) -> str:
    ...
    if with_image:
        ...
        head = [
            "你是电子实验室巡检的端侧判定器。下面是初筛(L1)按规则给出的结构化告警"
            "（含物体类别、温度、热点状态、现场上下文；没有图像）。初筛常有误报，"
            "你必须重新判定真实严重度，不要照搬初筛 severity。",
            "判据：① 常温或仅轻微发热的物体(如插排/适配器/电线，温度不高)即使被报警也判 info；"
            "② 高温危险源(电烙铁/焊台/热风枪等)在课后或无人看管时判 critical；"
            "③ 高温但课中有人正常操作判 warning。",
            "用一句中文说明情况并给出处置建议。若仅凭结构化信息无法判断、疑似视觉误报或需看图核实，"
            "把 escalate_to_cloud 设为 true。",
        ]
    lines = head + [
        "",
